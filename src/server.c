...
struct event_entry {
    ...
    TAILQ_ENTRY(event_entry) entries;
};
...
#else
#define CLIENT_DEBUG(client, message, ...)
#define CLIENT_INFO(client, message, ...)
...
void client_notify_characteristic(homekit_characteristic_t *ch, homekit_value_t value, void *context) {
    ...
    struct event_entry* elem = malloc(sizeof(struct event_entry));
    if (elem)
    {
        elem->event = event;
        TAILQ_INSERT_HEAD((&(client->event_queue)), elem, entries);
    }else
    {
...
void client_send_chunk(byte *data, size_t size, void *arg) {
    ...
    int offset = snprintf((char *)payload, payload_size, "%lx\r\n", size);
...
static bool bool_endpoint_param(const char *name, client_context_t* context) {
    query_param_t *param = query_params_find(context->endpoint_params, name);
    return param && param->value && !strcmp(param->value, "1");
}

static void write_characteristic_error(json_stream *json, int aid, int iid, int status) {
    json_object_start(json);
...
static HAPStatus process_characteristics_update(const cJSON *j_ch, client_context_t *context) {
...
void homekit_server_on_resource(client_context_t *context) {
    ...
    char* path = context->server->config->on_resource(context->body, context->body_length);
...
void homekit_server_process_notifications(homekit_server_t *server) {
    ...
    while (context) {
        ...
        if (!TAILQ_EMPTY(&(context->event_queue)))
        {
            struct event_entry *hev= context->event_queue.tqh_first;
            TAILQ_REMOVE(&(context->event_queue), context->event_queue.tqh_first, entries);
            event = hev->event;
            free(hev);
            ...
            while (!TAILQ_EMPTY(&(context->event_queue))) {
                struct event_entry *hev= context->event_queue.tqh_first;
                TAILQ_REMOVE(&(context->event_queue), context->event_queue.tqh_first, entries);
                event = hev->event;
                free(hev);
